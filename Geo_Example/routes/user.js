    ...
    if(database){
        authUser(database, paramId, paramPassword, function(err, docs)
        {
            ...
            if(docs){
                ...
                res.app.render('login_success',context, function(err,html){
                    if(err){
                        console.error('view 렌더링 중 에러 발생 : ' + err.stack);
                        console.log('에러 발생.');

                        res.writeHead(200,{'Content-Type':"text/html;charset=utf8"});
                        res.write('<h1>view 렌더링 중 에러 발생</h1>');
    ...
    if(database){
        database.UserModel.findAll(function(err, results){
            ...
            if(results){
                ...
                res.app.render('listuser',context, function(err,html){
                    if(err){
                        console.error('view 렌더링 중 에러 발생 : ' + err.stack);
                        console.log('에러 발생.');
